...
@Component({
  selector: 'app-report-approval-major',
  templateUrl: './report-approval-major.component.html',
  styleUrls: ['./report-approval-major.component.scss']
})
export class ReportApprovalMajorComponent implements OnInit {
  ...
  requestUrl: string = environment.api_url + this.reportService.path + '/approval_student_by_major';
  ...
  onLoad(): void {
    ...
    this.reportService
      .getApprovalStudentCountByMajor({ ...this.filterParams, start_date: startDate, end_date: endDate })
      .pipe(
        map(map => {
  ...
  trackByFn(index: number, item: any): void {
    ...
    const title = `ចំនួនអនុម័តតាមជំនាញចាប់ពី  ${startDate} ដល់ ${endDate}`;
